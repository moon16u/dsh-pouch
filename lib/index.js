...
// llm-model-listing and core-memory are local-only packages: they carry
// `private: true`, and root `files` keeps them out of the published tarball.
// So they are present in a checkout and absent from an installed copy, and a
// static import would turn every install into ERR_MODULE_NOT_FOUND.
...
let coreMemory;
try {
  coreMemory = await import("../packages/dsh-plugin-core-memory/lib/index.js");
} catch {
...
export function apply(ctx, config) {
  ...
  // Reads its own `dsh-core-memory` settings section (default: <cwd>/memory/core),
  // so it stays silent in workspaces without core memory. Absent from an
  // installed copy; see the guarded import above.
  if (coreMemory !== undefined) ctx.plugin(coreMemory);
