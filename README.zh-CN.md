...
* **版本跨度**：为 DSH `0.1.5-rc.1` 构建。设置页通过双代线缆读取提供方目录并写入设置——DSH 0.1.1 的 `connection.api`（APIProxy）或 `remote.llm` / `remote.settings` 命名空间——宿主提供哪个就用哪个。它必须遵守两条宿主契约：提供方 profile 校验的是路由的副本，settings provider 交出的 `deepFreeze` 段落因此保持原样；解析出的 profile 也带上 `dsh-llm-pi-ai` 的 `PiAiAdapter.modelOf` 读取的 `modelErrors` 映射。
